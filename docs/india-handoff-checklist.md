...
- ✅ Send/Resend intake link from the Lead (2026-07-15, answering India's question — there is ALWAYS a path):
  - New Lead: the "Send Intake Link" button on Lead Details always works — it opens the send doorway via the open Task when one exists, or directly when the Task was removed. Approving sends the Intake Form Link email, moves the Lead Stage to Intake Sent, completes any open send Task, and schedules the weekly follow-up (if that Task type is On)
  - Intake Sent: a "Resend Link" button next to the sent date opens the same reviewed draft; approving re-sends the identical link and logs the Communication + timeline entry — no Stage move, no new Task. Gone after submission (the form is locked)
  - Both paths go through the reviewed-draft doorway — nothing sends blind. The weekly Intake Follow-Up email also carries the link automatically
- ✅ Firm account rule (2026-07-13): every partner law firm has exactly ONE permanent firm-level user, created automatically with the tenant. Name = the firm (e.g., "McCune Legal"), Role = "Law Firm" (a distinct role — not Attorney, not Staff), Status = Active always. It cannot be edited, deactivated, or deleted from User Management. Every firm account has a single firm email address on the platform domain (pattern: {firm}@bkfastlane.com — demo: mccunelegal@bkfastlane.com). Rationale reserved (Matt) — the rule stands regardless
